...
public:
    DigitalIn_ESP8266(PhysicalPin pin) :
        _gpio(pin), _pinChange(DIN_NONE)
    {
    }
    ;
...
//       INFO(" DigitalIn Init %d ", _gpio);
        ...
        io_conf.pull_down_en =
            (gpio_pulldown_t) (_mode & DIN_PULL_DOWN ? 1 : 0);
...
#define ESP_INTR_FLAG_DEFAULT 0
        if (_fp) {
            ...
            if (_fp)
                erc = gpio_isr_handler_add((gpio_num_t) _gpio, _fp,
                                           (void*) _object);
            if (erc)
                ERROR("gpio_isr_handler_add() :%d", erc);
        }
...
Erc I2C_ESP8266::write(uint8_t b)
{
    return write(&b, 1);
}
...
//        check_efuse();
        ...
        if (_unit == ADC1) {
            ...
            erc = adc1_config_channel_atten((adc1_channel_t) _channel,
                                            ADC_ATTEN_DB_11);
            if (erc)
    ...
    int getValue()
    {
        if (_unit == ADC1) {
            ...
            int voltage;
            esp_err_t r = adc2_get_raw((adc2_channel_t) _channel,
                                       ADC_WIDTH_10Bit, &voltage);
            if (r == ESP_OK) {
                return voltage;
            } else {
...
public:
    ...
    Erc exchange(Bytes& in, Bytes& out)
    {
        ...
        if (true) {
            ...
            if (ret) {
                ERROR("spi_device_get_trans_result(_spi, &pTrans, 1000) = %d ",
                      ret);
                return EIO;
            }
        }
...
//       uart_config.use_ref_tick = false;
        ...
        if ( uart_driver_install(_uartNum, RX_BUF_SIZE, 0, 20, &_queue, 0) )  ERROR("uart_driver_install() failed.");
    ...
    void event_task()
    {
        ...
        for (;;) {
            ...
            if (xQueueReceive(_queue, (void* )&event,
...
//						INFO(" RXD 0x%X", dtmp[i]);
                ...
                case UART_PATTERN_DET: {
                    ...
                    INFO("[UART PATTERN DETECTED] pos: %d, buffered size: %d",
                         pos, buffered_size);
                    if (pos == -1) {
...
const char* sLogicalPin[] = { "TXD", "RXD", "SCL", "SDA", "MISO", "MOSI", "SCK",
                              "CS"
                            };
